...
#include <stdexcept>
#include <vector>
#include <origami/resources/subTexture.hpp>
...
SubTexture::SubTexture(std::filesystem::path filePath, std::shared_ptr<SuperTexture> super_) : super{super_} {
    // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);	
    // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
...
SubTexture::SubTexture(int width_, int height_, std::shared_ptr<SuperTexture> super_) : super{super_}, width{width_}, height{height_} {
    data = std::vector<ColourBGRA32>(width*height);
...
void SubTexture::Submit(components::Program program) {
    super->BindRaw();
